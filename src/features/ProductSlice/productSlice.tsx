...
export const fetchProducts = createAsyncThunk(
  ...
  async (filterValue: IFilterTypes) => {
    const response = await axios.get<IProduct[]>("https://baran-market-app-backend.herokuapp.com/items");
    ...
    const filteredProducts: IProductsState = {
      data: filteredData.slice(
        ...
        (filterValue.index - 1) * filterValue.pageSize + filterValue.pageSize
      ),
...
const productsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      ...
      (state, action: PayloadAction<IProductsState>) => {
